        ...
        for i in test_ids:
            if len(test_word_seqs[i]) < 2:
                if opt.split_one_arg:
                    ...
                    class_vec, type_vec, subtype_vec, relation_vec = model(('Eval', 'N/A', 0, 1), splitted)
                    relDump[i]=[torch.cat([class_vec, type_vec, subtype_vec, relation_vec.view(opt.hidden_dim*opt.num_direction*2)]).view(1, -1).cpu().numpy()]
                    msgAVGDump[i]=np.mean(relDump[i],axis=0)
                    skips.append(i)
                    continue
            ...
            for j in range(len(test_word_seqs[i])-1):
                class_vec, type_vec, subtype_vec, relation_vec = model(('Eval', 'N/A', j, j+1), test_word_seqs[i])
                try:
                    relDump[i].append(torch.cat([class_vec, type_vec, subtype_vec, relation_vec.view(opt.hidden_dim*opt.num_direction*2)]).view(1,-1).cpu().numpy())
                except KeyError:
                    relDump[i]=[torch.cat([class_vec, type_vec, subtype_vec, relation_vec.view(opt.hidden_dim*opt.num_direction*2)]).view(1, -1).cpu().numpy()]            
            msgAVGDump[i]=np.mean(relDump[i],axis=0)
