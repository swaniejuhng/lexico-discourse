import discourseParsing.DiscourseParser_Analysis as DP
...
def main():
    ...
    with torch.no_grad():
        ...
        for i in test_ids:
            if len(test_word_seqs[i]) < 2:
                if opt.split_one_arg:
                    ...
                    results = model((i,attn_dict,word_attn_dict,'Eval', 'N/A', 0, 1), splitted)
                    if results is None:
            ...
            for j in range(len(test_word_seqs[i])-1):
                results = model((i,attn_dict,word_attn_dict,'Eval', 'N/A', j, j+1), test_word_seqs[i])
                if results is None:
                    print("The message at '%d' doesn't have any word in the given embedding dict. Skipping this message"%(i))
                    continue
                else:
        ...
        pickle.dump(attn_dict,open('attnDict_'+opt.model.split('/')[-1]+opt.test_file.split('/')[-1]+'.dict','wb'))
        pickle.dump(word_attn_dict,open('wordAttnDict_'+opt.model.split('/')[-1]+opt.test_file.split('/')[-1]+'.dict','wb'))
